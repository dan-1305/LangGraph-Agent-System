...
def run_with_retry_and_log(cmd: list, cwd: str, timeout: int, max_retries: int = 2, job_name: str = "Job"):
    ...
    for attempt in range(1, max_retries + 1):
        ...
        try:
            ...
            # [CRITICAL] Luôn đảm bảo PYTHONPATH trỏ về root
            full_cmd = [sys.executable] + cmd if not str(cmd[0]).endswith('.exe') else cmd
            
            # log environment for debugging if needed
            # logger.debug(f"[{job_name}] PYTHONPATH: {current_env.get('PYTHONPATH')}")
...
def send_telegram_alert(message: str):
    # [FIX] Sử dụng đúng biến TELE_TOKEN từ .env
    token = os.getenv("TELE_TOKEN")
    chat_id = os.getenv("CHAT_ID")
    if not token or not chat_id:
        logger.warning("Không có TELE_TOKEN hoặc CHAT_ID để gửi cảnh báo.")
        return
...
def scrape_job():
    """Sync execution để schedule.CancelJob có thể propagate (Fix Bug #2).
    @run_queued wrapper nuốt return value khi submit vào async queue."""
    ...
    if success:
        ...
        err_msg = html.escape(str(err_log))
...
def airdrop_job():
    ...
    if success:
        ...
        # send_telegram_alert("✅ <b>[Airdrop Guerrilla]</b> Hoàn thành phiên cày cuốc On-chain thành công!") # Khóa lại để tránh trùng log báo cáo của bot con
    else:
...
if __name__ == "__main__":
    logger.info("==========================================================")
    logger.info("   🚀 SCHEDULER V2: CHẾ ĐỘ AIRDROP ONLY (TREO MÁY) 🚀")
    logger.info("==========================================================")
    ...
    logger.info(" (Mode: AIRDROP ONLY - Tắt hết các tác vụ khác để chống rác)\n")
