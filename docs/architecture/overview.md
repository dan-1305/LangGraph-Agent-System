...
- `APIGateway`: Centralized API Gateway & Rate Limiter & Circuit Breaker

**Các Hàm (Functions/Methods):**
- `APIGateway.__init__()`
- `APIGateway._init_db()`
- `APIGateway.check_circuit_breaker()`: Check if the circuit is open. If OPEN and recovery_timeout has passed, transition to HALF_OPEN.
- `APIGateway.record_success()`: Record a successful request to reset the circuit breaker.
- `APIGateway.record_failure()`: Record a failed request. Open the circuit if threshold is reached.
- `APIGateway._generate_hash()`
- `APIGateway._check_rate_limit()`: Check if the module has exceeded the rate limit in the last minute.
- `APIGateway.get_cached_response()`
- `APIGateway.save_to_cache()`
...
**Các Hàm (Functions/Methods):**
- `FileArchiverSkill.__init__()`
- `FileArchiverSkill._register()`
- `FileArchiverSkill.safe_archive()`
- `FileArchiverSkill.replace_in_file()`

### 📄 File: `src/tools/safe_codec_handler.py`
- Lỗi phân tích AST: unterminated string literal (detected at line 11) (safe_codec_handler.py, line 11)
...
- `MultiAgentTradingSystem._whale_tracker_node()`
- `MultiAgentTradingSystem._ml_node()`
- `MultiAgentTradingSystem._non_ai_fallback()`
- `MultiAgentTradingSystem._risk_manager_node()`
- `MultiAgentTradingSystem._start_node()`
...
**Các Lớp (Classes):**
- `LeftBrainPredictor`: Não Trái (XGBoost/LSTM) cho AI Trading Agent.

**Các Hàm (Functions/Methods):**
- `LeftBrainPredictor.__init__()`
- `LeftBrainPredictor._create_features()`: Feature Engineering (TA-Lib)
- `LeftBrainPredictor.train()`: Huấn luyện model dự đoán xu hướng tiếp theo (Up/Down)
- `LeftBrainPredictor.predict()`: Dự đoán xác suất tăng giá dựa trên dữ liệu gần nhất.
...
**Các Hàm (Functions/Methods):**
- `SelfReflectionAgent.__init__()`
- `SelfReflectionAgent.get_yesterday_trades()`
- `SelfReflectionAgent.get_current_prices()`
- `SelfReflectionAgent._ai_handler()`
- `SelfReflectionAgent._logic_handler()`
- `SelfReflectionAgent.run_daily_reflection()`
...
**Các Hàm (Functions/Methods):**
- `install_dependencies()`: Kiểm tra và cài đặt các dependency bị thiếu.
- `main()`
...
**Các Hàm (Functions/Methods):**
- `TTSEngine.__init__()`: Khởi tạo Edge-TTS engine.
- `TTSEngine._generate_audio_async()`
- `TTSEngine.generate_audio()`: Tạo file âm thanh MP3 từ đoạn text bằng Edge-TTS (Dùng giọng AI Microsoft).
...
**Các Hàm (Functions/Methods):**
- `_add_kpi_textbox()`: Adds a KPI textbox to a slide with main and sub text.
- `create_dashboard_ppt()`: Generates a PowerPoint presentation for real estate prediction project.
...
**Các Lớp (Classes):**
- `BacktestEngine`: Hệ thống Backtest tích hợp quản trị rủi ro:

**Các Hàm (Functions/Methods):**
- `BacktestEngine.__init__()`
- `BacktestEngine.run()`: Alias for run_backtest
- `BacktestEngine.run_backtest()`: signals: Mảng dự báo từ AI (1: Mua, 0: Không hành động)

### 📄 File: `projects/ĐỀ TÀI HỌC SÂU, kết hợp làm Đồ án môn AI & Học máy/Stock_Forecasting_Project/04_Source_Code/data_ingestion.py`
- **Mô tả File:** Không có mô tả.

**Các Lớp (Classes):**
- `DataIngestor`: Module chịu trách nhiệm thu thập và làm sạch dữ liệu chứng khoán.

**Các Hàm (Functions/Methods):**
- `DataIngestor.__init__()`
- `DataIngestor.fetch_stock_data()`: Lấy dữ liệu OHLCV và xử lý dữ liệu trống.
...
**Các Hàm (Functions/Methods):**
- `FeatureEngineer.__init__()`
- `FeatureEngineer.add_indicators()`
- `FeatureEngineer.create_sliding_windows()`
- `FeatureEngineer.time_series_split()`
...
**Các Lớp (Classes):**
- `NLPProcessor`: Xử lý phân tích cảm xúc tin tức bằng FinBERT.
- `DataFusionModel`: Kiến trúc kết hợp Chuỗi thời gian (LSTM) và Văn bản (FinBERT).
- `BacktestEngine`: Hệ thống đánh giá hiệu quả đầu tư giả lập.

**Các Hàm (Functions/Methods):**
- `NLPProcessor.__init__()`
- `NLPProcessor.get_sentiment()`: Trả về vector xác suất cảm xúc [Positive, Negative, Neutral].
- `DataFusionModel.build_model()`
- `BacktestEngine.__init__()`
- `BacktestEngine.run()`: Tính toán Sharpe Ratio và Drawdown.
...
**Các Hàm (Functions/Methods):**
- `patch_stdout_utf8()`: Ép xung sys.stdout để sử dụng UTF-8, khắc phục lỗi mã hóa trên một số hệ thống.
- `safe_patch()`: Thay thế một block code trong file bằng regex, hoặc tạo file nếu không tồn tại.
...
**Các Hàm (Functions/Methods):**
- `safe_patch_file()`
- `WorkspaceSanitizer.sanitize()`

### 📄 File: `core/skill_manager.py`
- **Mô tả File:** Không có mô tả.

**Các Hàm (Functions/Methods):**
- `_ensure_skills_directory_exists()`: Ensures the skills directory exists.
- `_read_skill_index()`: Reads the skill index file.
- `_write_skill_index()`: Writes the skill index file.
- `save_skill()`: Saves a code snippet as a skill in the skills directory and updates the index.
...
- `daily_summary_job()`
- `check_missed_jobs()`: Kiểm tra và chạy bù TẤT CẢ các tác vụ ngày dựa theo mốc giờ cấu hình.
...
**Các Lớp (Classes):**
- `SemanticRouter`: Bộ định tuyến thông minh, phân tích ý định user để mở rộng Query (Query Expansion).

**Các Hàm (Functions/Methods):**
- `SemanticRouter.__init__()`
- `SemanticRouter.route_and_expand()`
- `query_rag_return_context()`: Trả về string thay vì print ra console, phục vụ cho JIT Context Manager.
- `query_rag()`
