...
## 💻 BƯỚC 1.5: CẤU HÌNH PHẦN CỨNG (Quan trọng)

Hệ thống hiện tại đang chạy trên cấu hình:

- **CPU:** Intel Xeon E5 2676v3 (Cần tận dụng đa luồng hợp lý).
- **RAM:** 16GB + 8GB ảo(Tránh các tác vụ OOM bằng cách lọc file rác).
- **GPU:** AMD RX 580 8GB (Dùng cho các tác vụ Vision/Inference nếu cần).
- **Lưu trữ:** Mọi dữ liệu nặng nằm ở `D:\Users\Admin\Downloads\LangGraphStorage`.
...
1. **Giới hạn tài nguyên:** Cần tận dụng sức mạnh đa luồng của Xeon E5 nhưng phải kiểm soát RAM chặt chẽ (Tránh OOM). Các tác vụ ML nên set `n_jobs=4` đến `8` thay vì lạm dụng toàn bộ 24 threads gây treo OS.
2. **Chiến lược LLM:**
   ...
   - Luôn gọi qua proxy `langchain_openai`, KHÔNG dùng SDK cũ. -->Thông tin này đã cũ rồi
