...
- `CentralizedAPIGateway._initialize_db()`: Initializes the SQLite database for circuit breaker state and LLM caching.
...
- `CentralizedAPIGateway.get_cached_response()`: Lấy phản hồi từ cache SQLite.
- `CentralizedAPIGateway.save_to_cache()`: Lưu phản hồi vào cache SQLite.
...
- `BaseAgent.execute()`: Mô hình Cầu Dao Điện (Circuit Breaker Pattern) + Watchdog Guardian.
...
### 📄 File: `src/resume_flow.py`
- **Mô tả File:** Không có mô tả.

**Các Hàm (Functions/Methods):**
- `get_latest_checkpoint()`: Lấy Checkpoint state mới nhất từ SQLite
- `resume_graph()`
- `main()`
...
**Các Hàm (Functions/Methods):**
- `Config.initialize()`
- `Config.get_llm_credentials()`: Returns a list of available API credentials, prioritized for survival.
...
### 📄 File: `src/factory/nodes/architecture_critic.py`
- **Mô tả File:** Không có mô tả.

**Các Lớp (Classes):**
- `ArchitectureCritic`: Agent Architecture Critic:

**Các Hàm (Functions/Methods):**
- `ArchitectureCritic.__init__()`
- `ArchitectureCritic._ai_handler()`
- `ArchitectureCritic._logic_handler()`
...
- `OmniOverlord._ai_handler()`: Bắt buộc triển khai từ BaseAgent.
- `OmniOverlord._logic_handler()`: Bắt buộc triển khai từ BaseAgent.
- `OmniOverlord.check_market_pulse()`: Đọc tin tức và phân tích xem có biến cố khẩn cấp (Emergency) không.

### 📄 File: `src/factory/nodes/patch_generator.py`
- **Mô tả File:** Không có mô tả.

**Các Hàm (Functions/Methods):**
- `git_patch_node()`: Node tạo file .patch dựa trên Draft Code đã được sửa (và pass qua Sandbox).

### 📄 File: `src/factory/nodes/principles_gate.py`
- **Mô tả File:** Không có mô tả.

**Các Hàm (Functions/Methods):**
- `principles_gate_node()`: Sovereign Principles Gate.
...
### 📄 File: `src/factory/nodes/remediation_agent.py`
- **Mô tả File:** Không có mô tả.

**Các Lớp (Classes):**
- `RemediationAgent`: Tác nhân Vá lỗi Tự động (Auto-Remediation).

**Các Hàm (Functions/Methods):**
- `RemediationAgent.__init__()`
- `RemediationAgent._ai_handler()`
- `RemediationAgent._logic_handler()`
- `remediation_node()`
...
**Các Lớp (Classes):**
- `RouterDecision`
- `SemanticRouter`: Enterprise-Grade Semantic Router (LLM-based) with Zero-Guessing Logic.

**Các Hàm (Functions/Methods):**
- `SemanticRouter.__init__()`
- `SemanticRouter.route_query()`
- `SemanticRouter._ai_handler()`
- `SemanticRouter._logic_handler()`
- `router_node()`

### 📄 File: `src/factory/nodes/sandbox_validator.py`
- **Mô tả File:** Không có mô tả.

**Các Lớp (Classes):**
- `ValidationResult`
- `SandboxValidator`: Module cách ly để kiểm thử mã nguồn được sinh ra bởi Agent.

**Các Hàm (Functions/Methods):**
- `SandboxValidator.__init__()`
- `SandboxValidator.execute_test_sandbox()`: Thực thi test suite một cách bất đồng bộ bằng lệnh 'uv run pytest'.
- `sandbox_validator_node()`: Node LangGraph thực thi kiểm thử. 

### 📄 File: `src/factory/nodes/system_designer.py`
- **Mô tả File:** Không có mô tả.

**Các Lớp (Classes):**
- `SystemDesigner`: Agent System Designer:

**Các Hàm (Functions/Methods):**
- `SystemDesigner.__init__()`
- `SystemDesigner._ai_handler()`
- `SystemDesigner._logic_handler()`
- `SystemDesigner.generate_mermaid_map()`: Đọc báo cáo từ AID Taskforce và sinh biểu đồ Mermaid.
...
### 📄 File: `src/factory/nodes/workflow_agent.py`
- **Mô tả File:** Không có mô tả.

**Các Lớp (Classes):**
- `WorkflowDecision`
- `AIWorkflowAgent`: AI Workflow Agent: Tự động quyết định gọi cycle/chain nào từ Workflow Engine

**Các Hàm (Functions/Methods):**
- `AIWorkflowAgent.__init__()`
- `AIWorkflowAgent.decide_workflow()`
- `AIWorkflowAgent._ai_handler()`
- `AIWorkflowAgent._logic_handler()`
- `workflow_agent_node()`
...
### 📄 File: `src/scrapers/auto_repair_selector.py`
- **Mô tả File:** Không có mô tả.

**Các Lớp (Classes):**
- `SelectorPatch`
- `AutoRepairSelector`: 🧬 SELF-REPAIRING SCRAPER MODULE (Hero Product 1)

**Các Hàm (Functions/Methods):**
- `AutoRepairSelector.__init__()`
- `AutoRepairSelector._logic_handler()`
- `AutoRepairSelector._ai_handler()`
- `AutoRepairSelector.repair_and_patch()`: Thực hiện quy trình sửa lỗi và vá code tự động.
...
- `AirdropExecutor._ai_handler()`: Bắt buộc triển khai từ BaseAgent.
- `AirdropExecutor._logic_handler()`: Bắt buộc triển khai từ BaseAgent.
...
### 📄 File: `projects/airdrop_guerrilla/src/automation/multi_account.py`
- **Mô tả File:** Không có mô tả.

**Các Lớp (Classes):**
- `MultiAccountManager`: Quan ly da tai khoan (Multi-Account) cho Airdrop Sentinel.

**Các Hàm (Functions/Methods):**
- `MultiAccountManager.__init__()`
- `MultiAccountManager.get_account_profile_path()`: Lay duong dan thu muc profile cho mot account cu the.
- `MultiAccountManager.list_accounts()`: Liet ke danh sach cac account hien co.
- `MultiAccountManager.delete_account()`: Xoa mot account va du lieu profile di kem.
...
- `StealthBrowser.stealth_page()`: Bơm stealth xịn vào page và giả lập vân tay trình duyệt (Canvas, WebGL).
...
- `StealthBrowser.human_click()`: Giả lập thao tác click chuột của người dùng (có di chuyển).
...
- `EVMBase.get_gas_price()`: Lấy giá Gas hiện tại. Hỗ trợ Gas-Optimization: Chờ đợi gas thấp.
...
- `EVMBase.random_delay()`: [UPGRADE] Gaussian Random Delay de lach bo loc Sybil.
- `EVMBase.fragment_amount()`: Chia nho so luong token thanh cac phan ngau nhien de lam nhiễu analysis.
...
### 📄 File: `projects/airdrop_guerrilla/src/utils/proxy_manager.py`
- **Mô tả File:** Không có mô tả.

**Các Lớp (Classes):**
- `ProxyManager`: Quan ly danh sach Proxy cho quan doan Clone.

**Các Hàm (Functions/Methods):**
- `ProxyManager.__init__()`
- `ProxyManager._load_proxies()`: Nap danh sach proxy tu file.
- `ProxyManager.get_proxy_for_account()`: Lay proxy gan cho account. 
- `ProxyManager.add_proxy()`: Them proxy moi vao danh sach (Format: http://user:pass@host:port).

### 📄 File: `projects/airdrop_guerrilla/src/utils/stealth_vault.py`
- **Mô tả File:** Không có mô tả.

**Các Lớp (Classes):**
- `StealthVault`: He thong luu tru bao mat cach ly (Stealth Vault).

**Các Hàm (Functions/Methods):**
- `StealthVault.__init__()`
- `StealthVault._load_or_create_key()`: Khoi tao hoac nạp key ma hoa tu Master Key he thong.
- `StealthVault.encrypt_and_store()`: Ma hoa va luu tru du lieu.
- `StealthVault.decrypt_and_retrieve()`: Giai ma va lay du lieu.
...
### 📄 File: `projects/ai_trading_agent/__init__.py`
- **Mô tả File:** Không có mô tả.
...
### 📄 File: `projects/ai_trading_agent/src/analysis_to_social.py`
- **Mô tả File:** Không có mô tả.

**Các Lớp (Classes):**
- `MarketAnalysisReport`
- `AnalysisAgent`: Agent chuyen chuyen hoa cac tin hieu ky thuat thanh bai phan tich chuyen sau.

**Các Hàm (Functions/Methods):**
- `AnalysisAgent.__init__()`
- `AnalysisAgent._logic_handler()`
- `AnalysisAgent._ai_handler()`

### 📄 File: `projects/ai_trading_agent/src/analytics.py`
- **Mô tả File:** Không có mô tả.

**Các Lớp (Classes):**
- `Analytics`: [ROLE: SRE / QA Tester]

**Các Hàm (Functions/Methods):**
- `Analytics.__init__()`
- `Analytics._ai_handler()`: Bắt buộc triển khai từ BaseAgent.
- `Analytics._logic_handler()`: Bắt buộc triển khai từ BaseAgent.
...
**Các Lớp (Classes):**
- `BinanceExecutor`: [ROLE: SRE / QA Tester]

**Các Hàm (Functions/Methods):**
- `BinanceExecutor.__init__()`
- `BinanceExecutor._ai_handler()`: Bắt buộc triển khai từ BaseAgent - Không dùng trực tiếp LLM tại đây.
- `BinanceExecutor._logic_handler()`: Bắt buộc triển khai từ BaseAgent.
- `BinanceExecutor.get_current_portfolio()`: Lấy số dư hiện tại trên sàn.
- `BinanceExecutor._get_latest_atr()`: Lấy giá trị ATR_14 mới nhất từ database.
- `BinanceExecutor._calculate_smart_position_sizing()`: Tính toán Position Sizing thông minh.
- `BinanceExecutor.execute_allocation()`: Thực thi rebalance danh mục.
...
**Các Lớp (Classes):**
- `FundamentalAnalyzer`: [ROLE: Senior Backend Developer]

**Các Hàm (Functions/Methods):**
- `FundamentalAnalyzer.__init__()`
- `FundamentalAnalyzer._ai_handler()`: Bắt buộc triển khai từ BaseAgent.
- `FundamentalAnalyzer._logic_handler()`: Bắt buộc triển khai từ BaseAgent.
...
**Các Hàm (Functions/Methods):**
- `GenericAgent._ai_handler()`
- `GenericAgent._logic_handler()`
- `MultiAgentTradingSystem.__init__()`
- `MultiAgentTradingSystem._ai_handler()`: Implement abstract method from BaseAgent
- `MultiAgentTradingSystem._logic_handler()`: Implement abstract method from BaseAgent
- `MultiAgentTradingSystem._init_node()`: Node khởi tạo để kích hoạt luồng Fan-out
...
- `calculate_risk_parity_weights()`: Tính toán tỷ trọng theo phương pháp Risk Parity (Cân bằng rủi ro).
- `optimize_portfolio()`: Thực hiện tối ưu hóa danh mục. Hỗ trợ MPT (Sharpe) và Risk Parity.

### 📄 File: `projects/ai_trading_agent/src/profit_harvester.py`
- **Mô tả File:** Không có mô tả.

**Các Lớp (Classes):**
- `ProfitHarvester`: He thong thu hoach loi nhuan tu dong (Profit Harvest).

**Các Hàm (Functions/Methods):**
- `ProfitHarvester.__init__()`
- `ProfitHarvester.update_position()`: Cap nhat trang thai vi the va kiem tra diem chot loi.
- `ProfitHarvester.open_position()`
...
**Các Lớp (Classes):**
- `TechnicalEngine`: ⚡ SOVEREIGN HIGH-PERFORMANCE ENGINE ⚡

**Các Hàm (Functions/Methods):**
- `TechnicalEngine.__init__()`
- `TechnicalEngine.analyze_trend()`: Phân tích theo chuẩn Minervini Stage 2 (Uptrend mạnh).

### 📄 File: `projects/ai_trading_agent/src/telegram_intelligence.py`
- **Mô tả File:** Không có mô tả.

**Các Lớp (Classes):**
- `TelegramSentiment`
- `TelegramIntelligenceAgent`: Agent chuyen phan tich tam ly thi truong tu cac tin nhan Telegram.

**Các Hàm (Functions/Methods):**
- `TelegramIntelligenceAgent.__init__()`
- `TelegramIntelligenceAgent._logic_handler()`
- `TelegramIntelligenceAgent._ai_handler()`
- `main()`

### 📄 File: `projects/ai_trading_agent/src/validation_gate.py`
- **Mô tả File:** Không có mô tả.

**Các Lớp (Classes):**
- `ValidationGate`: 🛡️ SOVEREIGN VALIDATION GATE (Tier 1 Protection)

**Các Hàm (Functions/Methods):**
- `ValidationGate.__init__()`
- `ValidationGate.check_data_integrity()`: [VULN-003] Kiểm tra tính toàn vẹn của dữ liệu (Sanity Check).
- `ValidationGate.validate_proposal()`: Kiểm tra đề xuất lệnh dựa trên dữ liệu 7 ngày và Slippage Guard.
...
### 📄 File: `projects/asset_audit_taskforce/src/app.py`
- **Mô tả File:** Không có mô tả.

### 📄 File: `projects/asset_audit_taskforce/src/arb_session.py`
- **Mô tả File:** Không có mô tả.

**Các Lớp (Classes):**
- `ArchitectureReconstructionBoard`: Ban Dự án Tái thiết Kiến trúc (ARB):

**Các Hàm (Functions/Methods):**
- `ArchitectureReconstructionBoard.__init__()`
- `ArchitectureReconstructionBoard.run_full_session()`

### 📄 File: `projects/asset_audit_taskforce/src/auditor.py`
- Lỗi phân tích AST: expected an indented block after 'if' statement on line 12 (auditor.py, line 13)

### 📄 File: `projects/asset_audit_taskforce/src/detective.py`
- Lỗi phân tích AST: expected an indented block after 'if' statement on line 13 (detective.py, line 14)

### 📄 File: `projects/asset_audit_taskforce/src/judge_executor.py`
- **Mô tả File:** Không có mô tả.
...
### 📄 File: `projects/asset_audit_taskforce/src/scouter.py`
- Lỗi phân tích AST: expected an indented block after 'if' statement on line 13 (scouter.py, line 14)
...
**Các Lớp (Classes):**
- `TTSEngine`: Dong co chuyen doi van ban thanh giong noi (TTS) da ngon ngu cao cap.

**Các Hàm (Functions/Methods):**
- `TTSEngine.__init__()`
- `TTSEngine.generate_audio()`: Sinh ra file mp3 tu van ban bang Edge-TTS.
- `TTSEngine._apply_audio_effects()`: Ap dung cac hieu ung am thanh chuyen nghiep.
...
### 📄 File: `projects/auto_x_bot/src/browser_bot.py`
- **Mô tả File:** Không có mô tả.

**Các Lớp (Classes):**
- `XBrowserBot`: Agent tuong tac X (Twitter) qua trinh duyet de lach luat cam API.

**Các Hàm (Functions/Methods):**
- `XBrowserBot.__init__()`
- `XBrowserBot.post_tweet_browser()`: Dang tweet bang cach dieu khien trinh duyet.
- `XBrowserBot.interact_with_trends()`: Tuong tac voi cac xu huong de tang uy tin account.
...
### 📄 File: `projects/auto_x_bot/src/social_coordinator.py`
- **Mô tả File:** Không có mô tả.
...
- Lỗi phân tích AST: expected an indented block after 'if' statement on line 9 (x_api_client.py, line 10)
...
- `AdminSimulator._ai_handler()`
- `AdminSimulator._logic_handler()`
...
### 📄 File: `projects/ceo_agent/marketing_intel.py`
- **Mô tả File:** Không có mô tả.

**Các Lớp (Classes):**
- `MarketingInsight`
- `MarketingAgent`: Agent chuyen phan tich thi truong va sinh noi dung Marketing.

**Các Hàm (Functions/Methods):**
- `MarketingAgent.__init__()`
- `MarketingAgent._logic_handler()`
- `MarketingAgent._ai_handler()`
- `main()`

### 📄 File: `projects/FlowNSFW-main/scripts/demo.py`
- **Mô tả File:** FlowNSFW Demo - Interactive video NSFW detection demonstration.

**Các Hàm (Functions/Methods):**
- `extract_frames_from_video()`: Extract frames from video file.
- `load_frame_sequence()`: Load frames from directory.
- `preprocess_frames()`: Convert frames to model input tensor.
- `sliding_window_inference()`: Run sliding window inference.
- `print_result()`: Pretty print inference result.
- `main()`

### 📄 File: `projects/FlowNSFW-main/scripts/eval_multi_res.py`
- **Mô tả File:** Evaluate FlowNSFW at multiple resolutions.

**Các Hàm (Functions/Methods):**
- `eval_at_resolution()`: Evaluate at a single resolution.
- `main()`

### 📄 File: `projects/FlowNSFW-main/scripts/generate_figures.py`
- **Mô tả File:** Generate performance comparison figure for FlowNSFW.

### 📄 File: `projects/FlowNSFW-main/scripts/infer.py`
- **Mô tả File:** FlowNSFW inference — sliding-window, boxes like YOLO, per-frame labels.

**Các Hàm (Functions/Methods):**
- `find_videos()`
- `draw_box()`: Draw YOLO-style bounding box on image.
- `infer_video()`: Full sliding-window video classification + detection boxes.
- `main()`

### 📄 File: `projects/FlowNSFW-main/scripts/test_smoke.py`
- **Mô tả File:** Không có mô tả.

### 📄 File: `projects/FlowNSFW-main/scripts/train.py`
- **Mô tả File:** Train FlowNSFW — optical-flow NSFW detection.

**Các Hàm (Functions/Methods):**
- `collate_simple()`: Collate for balanced batch — all same resolution, handle boxes as list.
- `_cosine_lr()`
- `_set_lr()`
- `main()`

### 📄 File: `projects/FlowNSFW-main/src/flow_nsfw/balanced_sampler.py`
- **Mô tả File:** Balanced batch sampler — ensures each batch has NSFW + SFW pairs.

**Các Lớp (Classes):**
- `BalancedBatchSampler`: Each batch contains exactly one NSFW and one SFW sample.

**Các Hàm (Functions/Methods):**
- `BalancedBatchSampler.__init__()`
- `BalancedBatchSampler.__iter__()`
- `BalancedBatchSampler.__len__()`

### 📄 File: `projects/FlowNSFW-main/src/flow_nsfw/data.py`
- **Mô tả File:** Video clip dataset for FlowNSFW.

**Các Lớp (Classes):**
- `VideoClipDataset`: Yields temporal clips from video frame directories.

**Các Hàm (Functions/Methods):**
- `_read_img()`: Read image as RGB uint8, handling AVIF via ffmpeg fallback.
- `VideoClipDataset.__init__()`
- `VideoClipDataset.__len__()`
- `VideoClipDataset.__getitem__()`

### 📄 File: `projects/FlowNSFW-main/src/flow_nsfw/detection_head.py`
- **Mô tả File:** NSFW Detection Head — multi-scale, flow-gated, with sparse processing.

**Các Lớp (Classes):**
- `_DetectScale`: Single-scale detection block: conv refine → box + cls heads.
- `DetectionHead`: Multi-scale NSFW detection head with optional sparse processing.

**Các Hàm (Functions/Methods):**
- `_compute_foreground_mask()`: Simple foreground activation mask from feature energy.
- `_window_mask_regions()`: Identify windows that intersect with the foreground mask.
- `_DetectScale.__init__()`
- `_DetectScale.forward()`: Returns (B, 5+nc, H, W) logits.
- `DetectionHead.__init__()`
- `DetectionHead._apply_sparse()`: Apply detection only on foreground windows.
- `DetectionHead.forward()`: Returns per-scale raw detection tensors.

### 📄 File: `projects/FlowNSFW-main/src/flow_nsfw/encoder_unet.py`
- **Mô tả File:** 4-stage U-Net encoder — adapted from FlowEraser.

**Các Lớp (Classes):**
- `UNetEncoder`: 4-level conv encoder: stride 1→2→4→8, returns bottleneck + skips.

**Các Hàm (Functions/Methods):**
- `_conv_block()`
- `UNetEncoder.__init__()`
- `UNetEncoder.forward()`

### 📄 File: `projects/FlowNSFW-main/src/flow_nsfw/flow_net.py`
- **Mô tả File:** Flow estimation backends — with accelerated correlation.

**Các Lớp (Classes):**
- `_FlowHead`
- `FlowNet`: Scratch-learned correlation flow estimator (optimized).
- `RaftFlowNet`: Frozen RAFT-S optical flow. Runs on HR frames, downsamples to feat stride.

**Các Hàm (Functions/Methods):**
- `_build_corr()`: Correlation volume via unfold + batched matmul.
- `_FlowHead.__init__()`
- `_FlowHead.forward()`
- `FlowNet.__init__()`
- `FlowNet.forward()`
- `RaftFlowNet.__init__()`
- `RaftFlowNet.forward()`

### 📄 File: `projects/FlowNSFW-main/src/flow_nsfw/losses.py`
- **Mô tả File:** FlowNSFW loss functions.

**Các Lớp (Classes):**
- `LossWeights`

**Các Hàm (Functions/Methods):**
- `_ciou_loss()`: CIoU box loss for positive samples.
- `detection_loss()`: Per-scale detection loss.
- `video_cls_loss()`: Video-level cross-entropy.
- `temporal_box_loss()`: Penalize abrupt box changes between adjacent frames.
- `simple_detection_loss()`: Simplified detection loss using GT boxes from YOLO pseudo-labels.
- `flow_consistency_loss()`: Forward-backward flow consistency loss.
- `flow_smoothness_loss()`: Spatial smoothness of optical flow.

### 📄 File: `projects/FlowNSFW-main/src/flow_nsfw/model.py`
- **Mô tả File:** FlowNSFW — Optical-flow-guided video NSFW detection with Mamba SSM support.

**Các Lớp (Classes):**
- `FlowNSFW`: Video NSFW detection with optical flow and temporal modeling.

**Các Hàm (Functions/Methods):**
- `FlowNSFW.__init__()`
- `FlowNSFW.count_parameters()`
- `FlowNSFW._decode_predictions()`: Decode raw detection heads into boxes per scale.
- `FlowNSFW.forward()`: Args:

### 📄 File: `projects/FlowNSFW-main/src/flow_nsfw/pseudo_labeler.py`
- **Mô tả File:** YOLO pseudo-labeler — convert video frames to FlowNSFW training manifest.

**Các Hàm (Functions/Methods):**
- `_decode_avif_to_numpy()`: Convert AVIF to RGB numpy array via ffmpeg pipe.
- `_decode_image()`: Read any image as RGB numpy array.
- `find_frame_dirs()`: Find leaf directories containing image sequences.
- `label_video_frames()`: Run YOLO on frames, decode AVIF→numpy as needed.
- `build_manifest()`: Build full manifest from video directories.
- `main()`

### 📄 File: `projects/FlowNSFW-main/src/flow_nsfw/ssm_backend.py`
- **Mô tả File:** SSM backend — 3-tier fallback chain with CUDA acceleration.

**Các Lớp (Classes):**
- `_FallbackSSM`: Minimal SSM using torch.cumsum. Functionally correct, no CUDA kernel.

**Các Hàm (Functions/Methods):**
- `_FallbackSSM.__init__()`
- `_FallbackSSM.forward()`: x: (B, L, D)
- `create_ssm_layer()`: Create the best available SSM layer.

### 📄 File: `projects/FlowNSFW-main/src/flow_nsfw/temporal_sparse.py`
- **Mô tả File:** Sparse global temporal aggregator — with Mamba SSM backend.

**Các Lớp (Classes):**
- `_AttnBlock`
- `_TransformerBlock`
- `_MambaBlock`: SSM-based temporal block: O(N) complexity for long sequences.
- `_HybridBlock`: Attention for local (self + warped neighbors), Mamba for global context.
- `SparseGlobalTemporal`: Temporal aggregator: window-local + sparse-global across clip frames.

**Các Hàm (Functions/Methods):**
- `_topk_tokens()`
- `_AttnBlock.__init__()`
- `_AttnBlock.forward()`
- `_TransformerBlock.__init__()`
- `_TransformerBlock.forward()`
- `_MambaBlock.__init__()`
- `_MambaBlock.forward()`: x: (B, N, C). _kv_unused kept for API compatibility.
- `_HybridBlock.__init__()`
- `_HybridBlock.forward()`
- `SparseGlobalTemporal.__init__()`
- `SparseGlobalTemporal._tokens()`
- `SparseGlobalTemporal._restore()`
- `SparseGlobalTemporal._build_kv()`: Build KV tokens: self + flow-warped neighbors + top-K global.
- `SparseGlobalTemporal.forward()`

### 📄 File: `projects/FlowNSFW-main/src/flow_nsfw/utils.py`
- **Mô tả File:** Shared utilities — adapted from FlowEraser.

**Các Hàm (Functions/Methods):**
- `warp()`: Bilinear grid_sample warp.
- `resize_flow_sequence()`: Resize pixel-unit flow to target (H, W), rescaling magnitudes.

### 📄 File: `projects/FlowNSFW-main/src/flow_nsfw/__init__.py`
- **Mô tả File:** FlowNSFW — Optical-flow-guided video NSFW detection.
...
**Các Hàm (Functions/Methods):**
- `translation_worker()`: Luồng dịch ngầm (Thread) để tránh block UI.

### 📄 File: `projects/godot_translator/build_godot_exe.py`
- **Mô tả File:** Không có mô tả.

### 📄 File: `projects/godot_translator/main.py`
- Lỗi phân tích AST: expected an indented block after 'if' statement on line 10 (main.py, line 11)

### 📄 File: `projects/godot_translator/main_run.py`
- **Mô tả File:** Không có mô tả.

**Các Hàm (Functions/Methods):**
- `main()`: Entry point chạy ngầm Streamlit. Dùng để thay thế file .bat cho bản Build.

### 📄 File: `projects/godot_translator/test_translation.py`
- **Mô tả File:** Không có mô tả.

**Các Hàm (Functions/Methods):**
- `test_single_file_translation()`

### 📄 File: `projects/godot_translator/core/cyber_security.py`
- **Mô tả File:** Không có mô tả.

**Các Lớp (Classes):**
- `IntegrityGuard`: Hệ thống kiểm soát tính toàn vẹn (Integrity Guard) dựa trên tri thức từ Practical Reverse Engineering.
- `InjectionGuard`: Lớp phong thu chong Prompt Injection (Jailbreak) va danh cap thong tin.

**Các Hàm (Functions/Methods):**
- `IntegrityGuard.__init__()`
- `IntegrityGuard._calculate_hash()`: Tính toán mã băm SHA-256 của file.
- `IntegrityGuard.record_baseline()`: Ghi nhận mã băm gốc của các file quan trọng.
- `IntegrityGuard.verify_integrity()`: Kiểm tra xem file có bị thay đổi không.
- `IntegrityGuard.deploy_logic_mine()`: Triển khai 'Mìn logic' - Tự hủy phiên làm việc nếu vi phạm tính toàn vẹn.
- `InjectionGuard.__init__()`
- `InjectionGuard.is_safe()`: Kiem tra xem input cua nguoi dung co an toan khong.
- `InjectionGuard.sanitize_input()`: Lam sach input neu can thiet.
...
### 📄 File: `projects/godot_translator/core/gdre_downloader.py`
- **Mô tả File:** Không có mô tả.
...
### 📄 File: `projects/godot_translator/core/pack_manager.py`
- **Mô tả File:** Không có mô tả.

**Các Lớp (Classes):**
- `DRMError`: Exception ném ra khi phát hiện game bị khóa bản quyền.
- `PackManager`: Quản lý việc Unpack và Repack các file .exe / .pck của Godot.

**Các Hàm (Functions/Methods):**
- `PackManager.__init__()`
- `PackManager.detect_godot_version()`: Kiểm tra Magic Bytes để nhận diện Godot PCK format.
- `PackManager.unpack()`: Giải nén file .exe hoặc .pck ra thư mục tạm.
- `PackManager.test_run()`: Khởi chạy game (Sandbox Playtest) không chặn UI chính.
- `PackManager.repack()`: Đóng gói lại thư mục đã dịch thành file game mới sử dụng GDRE Tools.
- `PackManager.cleanup_temp()`: Dọn dẹp rác (Garbage Collection) khi xong hoặc khi lỗi.
...
### 📄 File: `projects/godot_translator/utils/ui_helper.py`
- **Mô tả File:** Không có mô tả.

**Các Hàm (Functions/Methods):**
- `ui_error_guard()`: Decorator de bảo vệ UI Streamlit khỏi các lỗi Backend.
...
**Các Lớp (Classes):**
- `VisionParserAgent`: [ROLE: AI Prompt Engineer / QA Tester]

**Các Hàm (Functions/Methods):**
- `VisionParserAgent.__init__()`
- `VisionParserAgent._ai_handler()`: Bắt buộc triển khai từ BaseAgent - Không dùng trong luồng này.
- `VisionParserAgent._logic_handler()`: Bắt buộc triển khai từ BaseAgent - Không dùng trong luồng này.
- `VisionParserAgent.parse_schedule_image()`: Sử dụng BaseAgent để gọi LLM Vision (qua Local Proxy) xử lý ảnh lịch.
- `parse_schedule_image()`: Wrapper function để không làm vỡ các module đang import hàm này.
...
### 📄 File: `projects/nsfw_multimedia_auditor/api.py`
- **Mô tả File:** Không có mô tả.
...
### 📄 File: `projects/nsfw_multimedia_auditor/auditor_agent.py`
- **Mô tả File:** Không có mô tả.

**Các Lớp (Classes):**
- `ShadowAuditorV2`: [ROLE: Shadow Auditor / Chaos Alchemist]

**Các Hàm (Functions/Methods):**
- `ShadowAuditorV2.__init__()`
- `ShadowAuditorV2._ai_handler()`
- `ShadowAuditorV2._logic_handler()`
- `ShadowAuditorV2.analyze_frames()`
- `ShadowAuditorV2._calculate_skin_percentage()`
- `ShadowAuditorV2.generate_report()`
- `ShadowAuditorV2.save_json()`: Lưu kết quả ra file JSON để làm input cho LLM.

### 📄 File: `projects/nsfw_multimedia_auditor/censorship_engine.py`
- **Mô tả File:** Không có mô tả.

**Các Lớp (Classes):**
- `CensorshipEngine`: [ROLE: Cyber Security / Chaos Alchemist]

**Các Hàm (Functions/Methods):**
- `CensorshipEngine.__init__()`
- `CensorshipEngine.censor_image()`: Phát hiện và che vùng nhạy cảm trong ảnh.

### 📄 File: `projects/nsfw_multimedia_auditor/collision_logic.py`
- **Mô tả File:** Không có mô tả.

**Các Lớp (Classes):**
- `CollisionLogic`: [ROLE: Chaos Engineer / Action Analyst]

**Các Hàm (Functions/Methods):**
- `CollisionLogic.__init__()`
- `CollisionLogic.detect_insertion_events()`
- `CollisionLogic._is_overlapping()`
- `CollisionLogic._generate_detailed_summary()`

### 📄 File: `projects/nsfw_multimedia_auditor/downloader.py`
- **Mô tả File:** Không có mô tả.

**Các Hàm (Functions/Methods):**
- `download_llm()`: Tải model TinyLlama siêu nhẹ để tóm tắt nội dung.

### 📄 File: `projects/nsfw_multimedia_auditor/identity_tracker.py`
- **Mô tả File:** Không có mô tả.

**Các Lớp (Classes):**
- `IdentityTracker`: [ROLE: Principal System Architect / Chaos Alchemist]

**Các Hàm (Functions/Methods):**
- `IdentityTracker.__init__()`
- `IdentityTracker.process_timeline()`: Đọc dữ liệu JSON và phân loại nhân vật.
- `IdentityTracker._update_profiles()`
- `IdentityTracker._generate_v4_summary()`

### 📄 File: `projects/nsfw_multimedia_auditor/interaction_detector.py`
- **Mô tả File:** Không có mô tả.

**Các Lớp (Classes):**
- `InteractionDetector`: [ROLE: God Eye / Security Auditor]

**Các Hàm (Functions/Methods):**
- `InteractionDetector.__init__()`
- `InteractionDetector.analyze_interactions()`
- `InteractionDetector._calculate_distance()`
- `InteractionDetector._check_overlap()`
- `InteractionDetector._summarize_log()`

### 📄 File: `projects/nsfw_multimedia_auditor/narrative_bridge.py`
- **Mô tả File:** Không có mô tả.

**Các Lớp (Classes):**
- `NarrativeBridge`: [ROLE: Narrative Architect / Storyteller]

**Các Hàm (Functions/Methods):**
- `NarrativeBridge.__init__()`
- `NarrativeBridge._generate_rule_based_script()`: Dùng logic quy tắc để viết kịch bản nếu AI bị chặn.
- `NarrativeBridge._ai_handler()`: Thử dùng AI với prompt siêu an toàn (Abstract Geometry).
- `NarrativeBridge._logic_handler()`: Fallback tối thượng: Dùng Rule Engine của chúng ta.
- `NarrativeBridge.run_synthesis()`

### 📄 File: `projects/nsfw_multimedia_auditor/processor.py`
- **Mô tả File:** Không có mô tả.

**Các Hàm (Functions/Methods):**
- `extract_keyframes()`: Trích xuất keyframes từ video mỗi X giây.

### 📄 File: `projects/nsfw_multimedia_auditor/synthesizer.py`
- **Mô tả File:** Không có mô tả.

**Các Hàm (Functions/Methods):**
- `synthesize_narrative()`: Đọc timeline JSON và dùng TinyLlama để viết tóm tắt diễn biến.

### 📄 File: `projects/nsfw_multimedia_auditor/video_brain.py`
- **Mô tả File:** Không có mô tả.

**Các Lớp (Classes):**
- `VideoBrain`: [ROLE: God Eye / Multimodal Architect]

**Các Hàm (Functions/Methods):**
- `VideoBrain.__init__()`
- `VideoBrain.analyze_frame()`: Hỏi bộ não về một frame cụ thể.
- `VideoBrain.describe_interaction()`: Phân tích chuỗi hành động giữa các frame.

### 📄 File: `projects/nsfw_multimedia_auditor/vision_describer.py`
- **Mô tả File:** Không có mô tả.

**Các Lớp (Classes):**
- `CharacterDescriber`: [ROLE: AI Prompt Engineer / Visual Expert]

**Các Hàm (Functions/Methods):**
- `CharacterDescriber.__init__()`
- `CharacterDescriber._ai_handler()`
- `CharacterDescriber._logic_handler()`
- `CharacterDescriber.describe_character()`: Gửi ảnh 'sạch' lên Gemini để lấy mô tả ngoại hình.

### 📄 File: `projects/nsfw_multimedia_auditor/visual_anatomy.py`
- **Mô tả File:** Không có mô tả.

**Các Lớp (Classes):**
- `VisualAnatomyEngine`: [ROLE: Principal System Architect / Chaos Alchemist]

**Các Hàm (Functions/Methods):**
- `VisualAnatomyEngine.analyze_appearance()`: Phân tích ngoại hình dựa trên các vùng đã phát hiện.
- `VisualAnatomyEngine._get_dominant_color_name()`: Xác định màu chủ đạo đơn giản.

### 📄 File: `projects/nsfw_multimedia_auditor/__init__.py`
- **Mô tả File:** NSFW Multimedia Auditor Module

### 📄 File: `projects/project_manager_sentry/governance_engine.py`
- **Mô tả File:** Không có mô tả.
...
**Các Hàm (Functions/Methods):**
- `SovereignGovernance.__init__()`
- `SovereignGovernance.check_edit_permission()`: Kiểm tra xem một Agent có quyền sửa file dựa trên OWNERS.json.
- `SovereignGovernance.lock_sovereign_project()`: Khóa các project L3 để ngăn chặn thay đổi ngoài ý muốn.

### 📄 File: `projects/project_manager_sentry/lifecycle_manager.py`
- **Mô tả File:** Không có mô tả.

**Các Hàm (Functions/Methods):**
- `evaluate_monorepo_health()`

### 📄 File: `projects/project_manager_sentry/manager_agent.py`
- **Mô tả File:** Không có mô tả.

**Các Lớp (Classes):**
- `ProjectManagerAgent`: Agent chuyên trách quản lý Monorepo Metadata và điều phối file.

**Các Hàm (Functions/Methods):**
- `ProjectManagerAgent.__init__()`
- `ProjectManagerAgent.find_core_files()`: Truy vấn metadata để lấy danh sách core files của một project.
...
**Các Hàm (Functions/Methods):**
- `main()`: Main function to run the SillyTavern World Card Generator workflow.
...
- `WorldCardGenerator._ai_handler()`
- `WorldCardGenerator._logic_handler()`
...
**Các Lớp (Classes):**
- `BaseGeminiAgent`: Class cơ sở đã được refactor để kế thừa trực tiếp từ BaseAgent chuẩn của hệ thống,

**Các Hàm (Functions/Methods):**
- `BaseGeminiAgent.__init__()`
- `BaseGeminiAgent._ai_handler()`
- `BaseGeminiAgent._logic_handler()`
- `BaseGeminiAgent._call_gemini()`: Gửi prompt đến LLM sử dụng hệ thống gọi API đã chuẩn hóa.
- `BaseGeminiAgent._parse_json_response()`: Sử dụng logic parse chuẩn từ BaseAgent.
...
### 📄 File: `projects/sovereign_academy/main.py`
- **Mô tả File:** Không có mô tả.
...
### 📄 File: `projects/sovereign_academy/one_click_learn.py`
- **Mô tả File:** Không có mô tả.

**Các Hàm (Functions/Methods):**
- `one_click_run()`

### 📄 File: `projects/sovereign_academy/tutor_agent.py`
- **Mô tả File:** Không có mô tả.
...
- `QuizQuestion`
- `Quiz`
- `CodeTutorAgent`: [ROLE: Code Tutor / Sovereign Academy]

**Các Hàm (Functions/Methods):**
- `CodeTutorAgent.__init__()`
- `CodeTutorAgent._ai_handler()`
- `CodeTutorAgent._logic_handler()`
- `CodeTutorAgent.get_random_core_file()`: Lấy ngẫu nhiên 1 file Python từ core/ hoặc core_utilities/
- `CodeTutorAgent.read_code()`
- `CodeTutorAgent.explain_code()`
- `CodeTutorAgent.generate_quiz()`

### 📄 File: `projects/sovereign_terminal/airdrop_mcp_farmer.py`
- **Mô tả File:** Không có mô tả.

**Các Hàm (Functions/Methods):**
- `send_telegram_alert()`
- `farm_faucet()`: Sử dụng Playwright MCP (qua mcp_client.py) để tự động hóa trình duyệt

### 📄 File: `projects/sovereign_terminal/daemon.py`
- **Mô tả File:** Không có mô tả.

**Các Hàm (Functions/Methods):**
- `send_telegram_message()`: Gửi tin nhắn qua Telegram dùng TELE_TOKEN và CHAT_ID.
- `task_morning_briefing()`: Chạy báo cáo buổi sáng.
- `run_forever()`: Chạy vòng lặp vô tận (Daemon) - Level 4 Event-Driven.
- `main()`

### 📄 File: `projects/sovereign_terminal/main.py`
- **Mô tả File:** Sovereign Terminal - Headless AI Agent.

**Các Hàm (Functions/Methods):**
- `print_banner()`: In banner khi khởi động.
- `handle_tool_calls()`: Xử lý tool calls từ AI, trả về kết quả cho AI.
- `chat_loop()`: Vòng lặp Chat REPL (Read-Eval-Print Loop).
- `async_main()`: Entry point.
- `main()`

### 📄 File: `projects/sovereign_terminal/__init__.py`
- **Mô tả File:** Sovereign Terminal - Headless AI Agent chạy độc lập, không cần Cline.

### 📄 File: `projects/sovereign_terminal/core/config.py`
- **Mô tả File:** Sovereign Terminal - Cấu hình API và Environment.

**Các Lớp (Classes):**
- `Config`: Cấu hình trung tâm cho Sovereign Terminal.

**Các Hàm (Functions/Methods):**
- `Config.validate()`: Kiểm tra xem API Key đã được cấu hình chưa.
- `Config.get_client_config()`: Trả về config cho OpenAI client.

### 📄 File: `projects/sovereign_terminal/core/mcp_client.py`
- **Mô tả File:** Không có mô tả.
...
**Các Hàm (Functions/Methods):**
- `MCPManager.__init__()`
- `MCPManager.load_config()`
- `MCPManager.connect_all()`
- `MCPManager.execute_tool()`
- `MCPManager.close()`

### 📄 File: `projects/sovereign_terminal/core/persona.py`
- **Mô tả File:** Sovereign Terminal - Persona Loader.

**Các Hàm (Functions/Methods):**
- `load_persona()`: Load System Prompt đồ sộ từ các file cốt lõi của hệ thống.

### 📄 File: `projects/sovereign_terminal/core/tools.py`
- **Mô tả File:** Sovereign Terminal - Tools (Function Calling).
...
- `trigger_factory_workflow()`: Kích hoạt LangGraph Factory Workflow (DevOps từ xa).
- `execute_tool()`: Thực thi tool theo tên và trả về kết quả.

### 📄 File: `projects/sovereign_terminal/core/__init__.py`
- **Mô tả File:** Sovereign Terminal Core Modules.

### 📄 File: `projects/sovereign_terminal/gateways/telegram_bot.py`
- **Mô tả File:** Không có mô tả.
...
### 📄 File: `projects/universal_game_vault/src/analyzer.py`
- Lỗi phân tích AST: expected an indented block after 'if' statement on line 16 (analyzer.py, line 17)

### 📄 File: `projects/universal_game_vault/src/dispatcher.py`
- Lỗi phân tích AST: expected an indented block after 'if' statement on line 16 (dispatcher.py, line 17)

### 📄 File: `projects/universal_game_vault/src/scraper.py`
- **Mô tả File:** Không có mô tả.
...
**Các Hàm (Functions/Methods):**
- `GameWebScraper.__init__()`
- `GameWebScraper._ai_handler()`
- `GameWebScraper._logic_handler()`
- `GameWebScraper.scrape_url()`: Cào nội dung văn bản từ một URL.

### 📄 File: `projects/universal_game_vault/src/processors/advanced_extractor.py`
- Lỗi phân tích AST: expected an indented block after 'if' statement on line 15 (advanced_extractor.py, line 16)

### 📄 File: `projects/universal_game_vault/src/processors/batch_importer.py`
- **Mô tả File:** Không có mô tả.
...
**Các Hàm (Functions/Methods):**
- `BatchImporter.__init__()`
- `BatchImporter._ai_handler()`: Thuc thi logic AI bat buoc tu BaseAgent
- `BatchImporter._logic_handler()`: Thuc thi logic Code bat buoc tu BaseAgent
- `BatchImporter.extract_text_from_pdf()`
- `BatchImporter.extract_text_from_docx()`
- `BatchImporter.run_import()`
- `BatchImporter.process_character_block()`: Su dung LLM de sinh file Wiki Markdown chuan.

### 📄 File: `projects/universal_game_vault/src/processors/full_text_parser.py`
- Lỗi phân tích AST: expected an indented block after 'if' statement on line 16 (full_text_parser.py, line 17)

### 📄 File: `projects/universal_game_vault/src/processors/lore_ingest_pipeline.py`
- **Mô tả File:** Không có mô tả.
...
**Các Hàm (Functions/Methods):**
- `LoreIngestionAgent.__init__()`
- `LoreIngestionAgent._ai_handler()`
- `LoreIngestionAgent._logic_handler()`
- `LoreIngestionAgent.save_to_db()`: Lưu dữ liệu đã trích xuất vào SQLite.
- `LoreIngestionAgent.ingest_from_url()`: Cào web và trích xuất dữ liệu.
- `LoreIngestionAgent.ingest_from_images()`: Chạy OCR trên thư mục ảnh và trích xuất dữ liệu.

### 📄 File: `projects/universal_game_vault/src/processors/text_parser.py`
- **Mô tả File:** Không có mô tả.
...
**Các Hàm (Functions/Methods):**
- `GameAIProcessor.__init__()`
- `GameAIProcessor.analyze_document()`: Sử dụng LLM để phân tích tài liệu và phân loại thực thể.
- `GameAIProcessor.update_wiki()`: Cập nhật các file Markdown trong thư mục wiki/.

### 📄 File: `projects/universal_game_vault/src/storage/db_manager.py`
- **Mô tả File:** Không có mô tả.
...
**Các Hàm (Functions/Methods):**
- `GameDBManager.__init__()`
- `GameDBManager.init_db()`: Khoi tao cau truc bang du lieu.
- `GameDBManager.sync_from_wiki()`: Doc file Markdown va dong bo vao SQLite.
- `GameDBManager.query_characters()`: Truy van nhan vat theo tieu chi.
...
### 📄 File: `core/logic_auditor.py`
- **Mô tả File:** Không có mô tả.

**Các Lớp (Classes):**
- `LogicAuditor`: Fitness Function - Bộ lọc đào thải tự nhiên của hệ thống Sovereign.

**Các Hàm (Functions/Methods):**
- `LogicAuditor.__init__()`
- `LogicAuditor.start_session()`: Bắt đầu đo lường tài nguyên trước khi Agent thực thi nháp.
- `LogicAuditor.evaluate_evolution()`: Đánh giá bản nháp so với tiêu chuẩn hệ thống.

### 📄 File: `core/mental_sandbox.py`
- **Mô tả File:** Không có mô tả.

**Các Lớp (Classes):**
- `MentalSandbox`: [ROLE: Principal System Architect / Chaos Engineer]

**Các Hàm (Functions/Methods):**
- `MentalSandbox.__init__()`
- `MentalSandbox._ensure_sandbox()`
- `MentalSandbox.create_draft()`: Tạo bản nháp của file trong sandbox.
- `MentalSandbox.verify_syntax()`: Kiểm tra lỗi cú pháp (Syntax Check) bằng AST.
- `MentalSandbox.verify_logic()`: [VULN-001] Kiểm tra các lỗi logic nguy hiểm (Runtime Risks).
- `MentalSandbox.run_unit_tests()`: Chạy các bài unit test liên quan.
- `MentalSandbox.deploy_draft()`: Merge bản nháp vào file gốc sau khi đã qua kiểm duyệt.
- `MentalSandbox.cleanup()`: Dọn dẹp sandbox.

### 📄 File: `core/post_mortem_engine.py`
- **Mô tả File:** Không có mô tả.

**Các Lớp (Classes):**
- `PostMortemEngine`: Module tự động phân tích lỗi lầm thực tế (Post-Mortem).

**Các Hàm (Functions/Methods):**
- `PostMortemEngine.__init__()`
- `PostMortemEngine.analyze_recent_trades()`: Phân tích các lệnh giao dịch gần đây.
- `PostMortemEngine._log_failure()`: Cập nhật vào FAILED_PATHS.json

### 📄 File: `core/principles_enforcer.py`
- **Mô tả File:** Không có mô tả.

**Các Lớp (Classes):**
- `PrinciplesEnforcer`: Lớp đối soát nguyên lý (Principles Enforcer) - Sovereign Core.

**Các Hàm (Functions/Methods):**
- `PrinciplesEnforcer.__init__()`
- `PrinciplesEnforcer.verify_decision()`: Kiểm tra xem quyết định có vi phạm nguyên lý cốt lõi không.
- `PrinciplesEnforcer.audit_agent_output()`: Chấm điểm độ tuân thủ nguyên lý của văn bản (0.0 - 1.0)

### 📄 File: `core/process_watchdog.py`
- **Mô tả File:** Không có mô tả.

**Các Lớp (Classes):**
- `ProcessWatchdog`: 💊 ĐẶC VỤ Y TẾ (Sovereign Process Watchdog)

**Các Hàm (Functions/Methods):**
- `ProcessWatchdog.__init__()`
- `ProcessWatchdog.log_failure()`: Ghi tội danh của tiến trình bị trảm vào FAILED_PATHS.json.
- `ProcessWatchdog.monitor_pid()`: Giám sát một tiến trình và TOÀN BỘ tiến trình con của nó.
...
**Các Lớp (Classes):**
- `DripFeedWorker`: 🌊 BỘ ĐIỀU PHỐI HỎA LỰC (Drip-Feed Worker)

**Các Hàm (Functions/Methods):**
- `DripFeedWorker.__init__()`
- `DripFeedWorker._throttled_task()`: Thực thi task với Semaphore khống chế.
- `DripFeedWorker.process_batch()`: Xử lý một danh sách các task theo kiểu nhỏ giọt (Drip-feed).
- `mock_scraping_task()`
- `test()`
...
- `scrape_job()`: Sync execution để schedule.CancelJob có thể propagate (Fix Bug #2).
- `daily_trading_job()`
- `airdrop_job()`
...
- Lỗi phân tích AST: expected an indented block after 'if' statement on line 62 (market_research_scraper.py, line 63)
...
### 📄 File: `tools/context_bridge/context_pipeline.py`
- **Mô tả File:** Context Pipeline V3.0 - Intelligence Layer cho Prepare Gemini

**Các Lớp (Classes):**
- `FileMeta`: Metadata cho mỗi file trong bundle.
- `FileTransformer`: Áp dụng per-file transformation rules để loại noise, compress dung lượng.
- `SectionEnricher`: Inject metadata header trước mỗi section để Gemini hiểu priority & context.

**Các Hàm (Functions/Methods):**
- `FileTransformer.transform()`: Transform entry point - dispatch theo file type.
- `FileTransformer._generic_clean()`: Làm sạch chung: blank lines, dashes, trailing whitespace.
- `FileTransformer._filter_error_encyclopedia()`: ERROR_ENCYCLOPEDIA rất dài do Auto-Generated AST section.
- `FileTransformer._minify_json()`: Minify JSON: bỏ indent thừa, giữ cấu trúc.
- `FileTransformer._strip_emoji_noise()`: GLOBAL_STATE: giữ emoji ở header nhưng collapse blocks thừa.
- `FileTransformer._collapse_decor()`: Clinerules: gộp các separator line dày đặc.
- `SectionEnricher.estimate_tokens()`: Ước lượng tokens (~4 chars/token cho mixed vi/en).
- `SectionEnricher.enrich()`: Build enriched section với header metadata.
- `transform_content()`: Public API: Transform content theo per-file rules.
- `enrich_section()`: Public API: Enrich section với metadata header.
- `get_file_meta()`: Lấy metadata của file từ registry.

### 📄 File: `tools/context_bridge/prepare_gemini_context.py`
- **Mô tả File:** Prepare Gemini Context V3.0 - Sovereign Context Pipeline

**Các Hàm (Functions/Methods):**
- `truncate_content()`: Cắt nội dung nếu vượt quá giới hạn ký tự, thêm thông báo.
- `read_file_safe()`: Đọc file an toàn, trả về (nội dung, số ký tự gốc).
- `read_user_input_tail()`: Đọc user input nhưng chỉ lấy N dòng cuối (Smart Filter).
- `gather_context()`: Gom toàn bộ context qua pipeline ETL V3.0.
- `main()`: Hàm main - parse CLI args, chạy pipeline và copy vào clipboard.

### 📄 File: `tools/context_bridge/__init__.py`
- **Mô tả File:** Context Bridge - Sovereign Context Pipeline cho external LLMs (Gemini/Claude/GPT)
...
### 📄 File: `tools/system/archivist_agent.py`
- **Mô tả File:** Không có mô tả.

**Các Lớp (Classes):**
- `ArchivistAgent`: Agent làm nhiệm vụ nén tri thức (Context Distillation).

**Các Hàm (Functions/Methods):**
- `ArchivistAgent.__init__()`
- `ArchivistAgent.distill_debate_room()`: Nén DEBATE_ROOM.md.
- `ArchivistAgent.run()`
...
### 📄 File: `tools/system/audit_reporter.py`
- **Mô tả File:** Không có mô tả.

**Các Lớp (Classes):**
- `AIAuditReporter`: He thong tu dong sinh bao cao PDF ve tinh trang suc khoe va loi nhuan cua Vuong trieu AI.

**Các Hàm (Functions/Methods):**
- `AIAuditReporter.__init__()`
- `AIAuditReporter._get_airdrop_stats()`
- `AIAuditReporter.generate_pdf()`

### 📄 File: `tools/system/auto_compliance_fixer.py`
- **Mô tả File:** Không có mô tả.

**Các Hàm (Functions/Methods):**
- `fix_missing_base_agent()`

### 📄 File: `tools/system/auto_import_fixer.py`
- **Mô tả File:** Không có mô tả.
...
### 📄 File: `tools/system/benchmark_core_functions.py`
- **Mô tả File:** Không có mô tả.
...
**Các Hàm (Functions/Methods):**
- `MockAgent.__init__()`
- `MockAgent._ai_handler()`
- `MockAgent._logic_handler()`
- `test_json_extraction()`
- `test_file_hashing()`
- `main()`

### 📄 File: `tools/system/benchmark_upgrades.py`
- **Mô tả File:** Không có mô tả.
...
**Các Hàm (Functions/Methods):**
- `test_router()`
- `get_current_weather()`: Tra ve thoi tiet hien tai cua mot thanh pho.
- `TestToolAgent.__init__()`
- `TestToolAgent._ai_handler()`
- `TestToolAgent._logic_handler()`
- `TestToolAgent.run_test()`
- `test_tool_calling()`
- `test_compressor()`
- `generate_report()`
...
### 📄 File: `tools/system/ceo_training_arena.py`
- **Mô tả File:** Không có mô tả.

**Các Lớp (Classes):**
- `CEOTrainingArena`: Môi trường huấn luyện chuyên biệt dành cho CEO (Cline).

**Các Hàm (Functions/Methods):**
- `CEOTrainingArena.__init__()`
- `CEOTrainingArena.start_exam()`

### 📄 File: `tools/system/chaos_monkey.py`
- **Mô tả File:** Không có mô tả.

**Các Lớp (Classes):**
- `ChaosMonkey`: [ROLE: QA Chaos Overlord]

**Các Hàm (Functions/Methods):**
- `ChaosMonkey.__init__()`
- `ChaosMonkey.simulate_memory_leak()`: Giả lập việc nuốt RAM để test Watchdog.
- `ChaosMonkey.simulate_api_blackout()`: Giả lập việc mất kết nối API bằng cách tạm thời ghi đè URL trong .env
- `ChaosMonkey.corrupt_database()`: Tiêm dữ liệu rác vào Database để test Validation Gates.

### 📄 File: `tools/system/chaos_monkey_v2.py`
- **Mô tả File:** Không có mô tả.

**Các Lớp (Classes):**
- `ChaosMonkeyV2`: [PHASE 1 Q4 - AUTOMATED CHAOS]

**Các Hàm (Functions/Methods):**
- `ChaosMonkeyV2.__init__()`
- `ChaosMonkeyV2.unleash_chaos()`
...
### 📄 File: `tools/system/cline_auto_clicker.py`
- **Mô tả File:** Không có mô tả.

**Các Hàm (Functions/Methods):**
- `main()`

### 📄 File: `tools/system/compliance_checker.py`
- **Mô tả File:** Không có mô tả.

**Các Lớp (Classes):**
- `ComplianceChecker`: [ROLE: SRE / QA Tester]

**Các Hàm (Functions/Methods):**
- `ComplianceChecker.__init__()`
- `ComplianceChecker._load_manifest_projects()`
- `ComplianceChecker.is_agent_file()`: Heuristic to determine if a file is intended to be an Agent.
- `ComplianceChecker.check_file()`
- `ComplianceChecker._get_project_name()`
- `ComplianceChecker._check_dangerous_nodes()`: [VULN-002] Phát hiện các nỗ lực lách luật bằng getattr hoặc import thư viện cấm.
- `ComplianceChecker._analyze_class()`
- `ComplianceChecker.run()`
- `ComplianceChecker.export_report()`

### 📄 File: `tools/system/context_compressor.py`
- **Mô tả File:** Không có mô tả.

**Các Hàm (Functions/Methods):**
- `compress_context()`: Nén nội dung ACTIVE_THOUGHTS.md bằng cách lưu trữ các task đã hoàn thành.
- `compress_text()`: Tiện ích nén Token (Tokenomics Utility):
...
### 📄 File: `tools/system/encyclopedia_builder.py`
- **Mô tả File:** Không có mô tả.

**Các Hàm (Functions/Methods):**
- `build_encyclopedia()`

### 📄 File: `tools/system/endtask.py`
- **Mô tả File:** Không có mô tả.

**Các Hàm (Functions/Methods):**
- `run_command()`
- `cleanup_processes()`: Dọn dẹp các tiến trình python/uv mồ côi để giải phóng venv lock.
- `has_changes()`: Kiểm tra xem có thay đổi quan trọng trong mã nguồn không (Git check).
- `anchoring()`: Sovereign Anchoring System (SAS)

### 📄 File: `tools/system/entity_tracker.py`
- **Mô tả File:** Không có mô tả.
...
**Các Hàm (Functions/Methods):**
- `EntityTracker.__init__()`
- `EntityTracker._ensure_map_exists()`
- `EntityTracker.load_map()`
- `EntityTracker.save_map()`
- `EntityTracker.track_path()`

### 📄 File: `tools/system/extract_v4.py`
- **Mô tả File:** Không có mô tả.
...
### 📄 File: `tools/system/fetch_free_models.py`
- **Mô tả File:** Không có mô tả.

**Các Hàm (Functions/Methods):**
- `fetch_free_models()`

### 📄 File: `tools/system/fix_pytest_buffer.py`
- **Mô tả File:** Không có mô tả.

**Các Hàm (Functions/Methods):**
- `fix_sys_stdout_buffer()`: Quét và sửa lỗi `sys.stdout = io.TextIOWrapper(...)` gây lỗi buffer detached cho Pytest.
...
### 📄 File: `tools/system/global_function_auditor.py`
- **Mô tả File:** Không có mô tả.
...
- `parse_file()`
- `generate_registry()`
- `performance_profiler()`: Decorator dùng để đo thời gian, mem (giả lập) của 1 hàm bất kỳ.
- `main()`

### 📄 File: `tools/system/health_check.py`
- **Mô tả File:** Không có mô tả.
...
**Các Hàm (Functions/Methods):**
- `SovereignHealthCheck.__init__()`
- `SovereignHealthCheck.check_venv_lock()`: Kiểm tra xem có tiến trình nào đang chiếm dụng .venv không.
- `SovereignHealthCheck.verify_dependencies()`: Kiểm tra xem các thư viện core có load được không.
- `SovereignHealthCheck.check_hardware_resources()`: Kiểm tra RAM và Disk.
- `SovereignHealthCheck.run_all_checks()`

### 📄 File: `tools/system/intelligence_audit.py`
- **Mô tả File:** Không có mô tả.

**Các Hàm (Functions/Methods):**
- `run_intelligence_audit()`

### 📄 File: `tools/system/log_siphoner.py`
- **Mô tả File:** Không có mô tả.
...
### 📄 File: `tools/system/next_focus.py`
- **Mô tả File:** Next Focus Generator - Self-Continuation Prompt cho Auto-Pilot trong 1 session.

**Các Hàm (Functions/Methods):**
- `load_session_state()`: Load session counter từ file JSON.
- `save_session_state()`: Persist session counter.
- `extract_pending_tasks()`: Trích xuất task chưa hoàn thành từ file markdown.
- `extract_next_steps_from_thoughts()`: Trích xuất section 'NEXT STEPS' từ ACTIVE_THOUGHTS.md.
- `generate_focus_prompt()`: Generate compact prompt (~5 dòng) cho next task.
- `main()`

### 📄 File: `tools/system/pre_flight_check.py`
- **Mô tả File:** Không có mô tả.

**Các Lớp (Classes):**
- `PreFlightCheck`: 🛩️ CỔNG KIỂM DUYỆT SỚM (Pre-flight Check)

**Các Hàm (Functions/Methods):**
- `PreFlightCheck.__init__()`
- `PreFlightCheck.check_zero_byte_files()`: Quét các file 0-byte (lỗi gãy stream).
- `PreFlightCheck.check_circular_imports_static()`: Quét lỗi import vòng lặp sơ bộ qua AST (không nạp module).
- `PreFlightCheck.run_static_checks()`

### 📄 File: `tools/system/project_indexer.py`
- **Mô tả File:** Không có mô tả.

**Các Hàm (Functions/Methods):**
- `analyze_dependencies()`: Phân tích các lệnh import để tìm sự phụ thuộc giữa các project.
- `index_monorepo_v2()`
...
- `parse_pdf_to_text()`: Parse PDF sang text bằng pymupdf4llm hoặc fitz.
- `main()`: mode: "system" (Code/Docs) hoặc "knowledge" (Sách/Papers)

### 📄 File: `tools/system/rag_query.py`
- **Mô tả File:** Không có mô tả.

**Các Hàm (Functions/Methods):**
- `query_rag_return_context()`: Hàm helper dùng cho các Agent để lấy context từ RAG.
- `main()`

### 📄 File: `tools/system/reading_task.py`
- **Mô tả File:** Không có mô tả.

**Các Lớp (Classes):**
- `LibrarianAgent`: Agent phụ trách việc đọc sách, tóm tắt và cập nhật vào bộ nhớ dài hạn.

**Các Hàm (Functions/Methods):**
- `LibrarianAgent.__init__()`
- `LibrarianAgent._ai_handler()`
- `LibrarianAgent._logic_handler()`
- `LibrarianAgent.read_random_wisdom()`: Chọn ngẫu nhiên một mẩu tri thức từ Knowledge DB và học tập.
...
### 📄 File: `tools/system/resource_monitor.py`
- **Mô tả File:** Không có mô tả.

**Các Hàm (Functions/Methods):**
- `get_system_stats()`: Thu thập thông tin tài nguyên phần cứng.
- `update_global_state()`: Cập nhật tài nguyên vào GLOBAL_STATE.md.
...
### 📄 File: `tools/system/run_simulator.py`
- **Mô tả File:** Không có mô tả.
...
**Các Lớp (Classes):**
- `TripleFilterCleaner`: Hệ thống thanh lọc 3 lớp (Triple-Filter Protocol):

**Các Hàm (Functions/Methods):**
- `TripleFilterCleaner.level_1_migrate()`: Di chuyển file/thư mục vào khu vực cách ly (Quarantine).
- `TripleFilterCleaner.level_2_compress()`: Nén các file trong quarantine đã cũ hơn X ngày.
- `TripleFilterCleaner.level_3_terminate()`: Xóa vĩnh viễn các bản nén cũ hơn X ngày.
- `clean_zombie_files()`: Hàm quyét và dọn dẹp các file rác đã biết.

### 📄 File: `tools/system/test_workflow_suite.py`
- **Mô tả File:** Workflow Evaluation Suite - Test toàn bộ 14 cycles + edge cases.

**Các Hàm (Functions/Methods):**
- `run_test()`: Chạy 1 test và thu thập kết quả.
- `create_temp_bad_file()`: Tạo file Python có nhiều issue.
- `create_empty_file()`: Tạo file rỗng.
- `create_large_file()`: Tạo file 500+ dòng.
- `main()`
- `generate_report()`: Tạo báo cáo MD.

### 📄 File: `tools/system/ultimate_doc_gen.py`
- **Mô tả File:** Không có mô tả.

**Các Hàm (Functions/Methods):**
- `generate_docs_and_map_models()`

### 📄 File: `tools/system/workflow_db.py`
- **Mô tả File:** Workflow DB - SQLite layer cho Sovereign Workflow Engine.

**Các Lớp (Classes):**
- `WorkflowDB`: SQLite CRUD layer cho Workflow Registry.

**Các Hàm (Functions/Methods):**
- `WorkflowDB.__init__()`
- `WorkflowDB._connect()`
- `WorkflowDB.init_schema()`: Khoi tao schema neu chua co.
- `WorkflowDB.register_cycle()`: Dang ky hoac update 1 cycle.
- `WorkflowDB.get_cycle()`: Lay thong tin 1 cycle.
- `WorkflowDB.list_cycles()`: Lay danh sach cycles.
- `WorkflowDB.log_execution()`: Ghi log 1 execution. Return execution ID.
- `WorkflowDB.update_execution()`: Update execution sau khi chay xong.
- `WorkflowDB.get_execution_history()`: Lay lich su executions.
- `WorkflowDB.get_stats()`: Thong ke tong quan.
- `seed_default_cycles()`: Seed DB voi 14 cycles mac dinh.

### 📄 File: `tools/system/workflow_engine.py`
- **Mô tả File:** Sovereign Workflow Engine - Orchestrator cho cycle registry.

**Các Lớp (Classes):**
- `WorkflowEngine`: Orchestrator: goi cycles, chain, log ket qua.

**Các Hàm (Functions/Methods):**
- `WorkflowEngine.__init__()`
- `WorkflowEngine.list_cycles()`: Hien thi registry.
- `WorkflowEngine.run_cycle()`: Chay 1 cycle don le.
- `WorkflowEngine.run_chain()`: Chay 1 chain (nhieu cycles lien tiep).
- `WorkflowEngine._execute_cycle_logic()`: Logic thuc thi cho tung cycle. CO TIMEOUT GUARD (300s).
- `WorkflowEngine._builtin_self_review()`: Tu review 1 file Python.
- `WorkflowEngine._builtin_codebase_audit()`: Audit kien truc: dem file, kiem tra cac thu muc core.
- `WorkflowEngine._builtin_security_scan()`: Quet loi bao mat co ban bang Regex (API keys, eval/exec).
- `WorkflowEngine._builtin_context_optimize()`: Kiem tra va de xuat nen context.
- `WorkflowEngine._builtin_rag_ingest()`: Goi script rag_ingest.py de cap nhat ChromaDB.
- `WorkflowEngine._builtin_kaizen_evolution()`: Thuc thi chu trinh Kaizen 1%: sinh test, viet nhat ky phan tinh.
- `WorkflowEngine._builtin_project_eval()`: Danh gia health 1 project.
- `WorkflowEngine._builtin_regression_guard()`: Chay pytest de kiem tra regression.
- `WorkflowEngine._builtin_compliance_check()`: Kiem tra compliance co ban.
- `WorkflowEngine._builtin_token_audit()`: Kiem tra token usage ( heuristic ).
- `WorkflowEngine._builtin_failure_memory()`: Doc FAILED_PATHS.json de nho loi cu.
- `WorkflowEngine.show_history()`: Hien thi execution history.
- `WorkflowEngine.show_stats()`: Hien thi thong ke.
- `WorkflowEngine._parse_chain_from_description()`: Parse chain cycle IDs tu description.
- `main()`
