...
Quản lý kết nối và các tác vụ đọc ghi vào CSDL SQLite trung tâm (`system_logs.db`).
Module này áp dụng cơ chế WAL (Write-Ahead Logging) để hỗ trợ đa luồng.
"""
...
class SystemDB:
    """
    Lớp quản lý Database SystemDB.
    
    Cung cấp các API lưu trữ nhật ký giao dịch, tracking token, và lỗi hệ thống.
    
    Attributes:
        db_path (str): Đường dẫn tuyệt đối đến file SQLite.
        conn (sqlite3.Connection): Đối tượng kết nối CSDL.
    """
    def __init__(self):
