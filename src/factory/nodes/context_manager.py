...
def context_manager_node(state: FactoryState):
    ...
    elif route == "ROUTE_CODEBASE_AUDIT":
        print(f"[Context Manager] Đang truy xuất Metadata Codebase Audit...")
        # Lấy file path (ví dụ user_query: "Hãy audit project X")
        # Ta tạm thời giả lập Cào 3 file cốt lõi của project đó (do không dùng RAG/DB Metadata ở đây mà đọc list thủ công)
        # Nếu tích hợp sâu, có thể query từ SQLite Metadata DB
        project_name = state.get("project_name", "LangGraph_Agent_System")
        
        injected_context = f"""
        [PROJECT METADATA DUMP: {project_name}]
        Dưới đây là một số thông tin cấu trúc mẫu để bạn Audit:
        File 1: `src/factory/main.py` -> Định nghĩa MetaGraph bằng LangGraph, tổ chức luồng điều phối.
        File 2: `src/base_agent.py` -> Tầng Transport bọc LLM Call, có Circuit Breaker và Graceful Degradation.
        File 3: `tests/test_resilience.py` -> Mock Unit test.
        """
        print("[Context Manager] Đã nạp Metadata dự án.")
