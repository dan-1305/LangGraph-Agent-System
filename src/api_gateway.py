...
class CentralizedAPIGateway:
    ...
    def _initialize_db(self):
        """Initializes the SQLite database for circuit breaker state and LLM caching."""
        ...
        """)
        # Bảng Cache cho API Gateway để lưu token theo chiến lược Cost-Effective
        cursor.execute("""
    ...
    def get_cached_response(self, prompt: str, schema_str: str = "") -> Optional[str]:
        """Lấy phản hồi từ cache SQLite."""
        import hashlib
        prompt_hash = hashlib.md5((prompt + schema_str).encode('utf-8')).hexdigest()
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        cursor.execute("SELECT response, timestamp FROM llm_cache WHERE prompt_hash = ?", (prompt_hash,))
    ...
    def save_to_cache(self, prompt: str, response: str, schema_str: str = ""):
        """Lưu phản hồi vào cache SQLite."""
        import hashlib
        prompt_hash = hashlib.md5((prompt + schema_str).encode('utf-8')).hexdigest()
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        cursor.execute(
            "INSERT OR REPLACE INTO llm_cache (prompt_hash, response, schema_str, timestamp) VALUES (?, ?, ?, ?)",
            (prompt_hash, response, schema_str, time.time())
        )
        conn.commit()
        conn.close()
