...
OUTPUT_FILE = "docs/QTKDCKS/Podcast_QTKDCKS_gTTS.mp3"

def create_audio():
    print("Khoi tao gTTS va ghep noi truc tiep (khong dung pydub)...")
    ...
    with open(OUTPUT_FILE, 'wb') as outfile:
        for i, text in enumerate(TEXT_CHUNKS):
            ...
            try:
                ...
                tts.save(chunk_file)
                # Đọc nội dung MP3 và ghi nối vào file đích
                with open(chunk_file, 'rb') as infile:
                    outfile.write(infile.read())
            except Exception as e:
                print(f"Loi render: {e}")
            finally:
