...
class StealthBrowser(BaseAgent):
    ...
    async def stealth_page(self, page: Page):
        """Bơm stealth xịn vào page và giả lập vân tay trình duyệt (Canvas, WebGL)."""
        try:
            # Bơm stealth tiêu chuẩn
            ...
            # [UPGRADE] Giả lập vân tay (Fingerprinting Bypass)
            # Override WebDriver để Cloudflare không phát hiện browser automation
    ...
    async def human_click(self, page: Page, selector: str):
        """Giả lập thao tác click chuột của người dùng (có di chuyển)."""
        element = await page.wait_for_selector(selector)
        if element:
            box = await element.bounding_box()
            if box:
                # Di chuyển chuột đến vị trí ngẫu nhiên trong phần tử
