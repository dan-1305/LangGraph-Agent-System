...
# [FIX] Tránh sử dụng sys.path.append, ưu tiên import tuyệt đối hoặc PYTHONPATH=.
# Thêm root dự án vào sys.path nếu cần thiết (dành cho môi trường đóng gói)
...
try:
    ...
    # Fallback cho trường hợp chạy độc lập hoặc đóng gói đặc biệt
...
class GodotTranslator(BaseAgent):
    ...
    def __init__(self, model_name: str = "gemini-3.1-flash-lite", api_key: str = None, **kwargs):
        # [FIX] Cho phép truyền trực tiếp API Key từ UI để không phụ thuộc .env/Proxy
        if api_key:
            kwargs['api_key'] = api_key
            # Nếu có API Key trực tiếp, không sử dụng Proxy
