...
tab1, tab2, tab3 = st.tabs(["📊 PnL & Portfolio", "🧠 AI Decisions", "📈 Market Indicators"])
...
with tab3:
    st.subheader("Trạng thái thị trường hiện tại (Market Indicators)")
    if not market_db_path.exists():
        st.warning("Chưa có dữ liệu Market. Vui lòng chạy data_fetcher.py")
    else:
        conn = sqlite3.connect(market_db_path, timeout=20.0)
        conn.execute('PRAGMA journal_mode=WAL;')
        try:
            df_btc = pd.read_sql("SELECT * FROM BTC_USD ORDER BY Date DESC LIMIT 10", conn)
            st.write("Dữ liệu BTC-USD 10 ngày gần nhất (Có kèm ATR, FNG):")
            st.dataframe(df_btc[['Date', 'Close', 'SMA_20', 'RSI_14', 'MACD', 'ATR_14', 'FNG_Value', 'FNG_Class']])
            
            # Vẽ biểu đồ RSI & FNG
            fig_rsi = px.line(df_btc, x='Date', y='RSI_14', title='RSI 14 Days (BTC)', markers=True)
            fig_rsi.add_hline(y=70, line_dash="dash", line_color="red")
            fig_rsi.add_hline(y=30, line_dash="dash", line_color="green")
            st.plotly_chart(fig_rsi, use_container_width=True)
            
        except Exception as e:
            st.error(f"Lỗi khi đọc market_db: {e}")
        finally:
            conn.close()
