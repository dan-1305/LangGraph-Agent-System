...
def fetch_cointelegraph_news(limit=5):
    ...
    try:
        ...
        # 🐳 TÍCH HỢP WHALE ALERT (Thực)
        from whale_alert import WhaleAlertMonitor
        monitor = WhaleAlertMonitor()
        summary = monitor.get_whale_alert_summary(assets=["bitcoin", "ethereum", "solana"])
        
        if "No Whale Alert API key" not in summary:
            news_list.append(summary)
        else:
            # Fallback nếu không có Key
            news_list.append("[10 Mar] 🚨 WHALE ALERT (Simulated): Một địa chỉ ví Cá voi bí ẩn vừa rút 5,000 BTC từ Coinbase về ví lạnh để Hold dài hạn.")
