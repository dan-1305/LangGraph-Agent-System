...
class BaseGeminiAgent:
    """
    Class cơ sở chứa logic gọi API thông qua ChatOpenAI (tương thích proxy của OpenAI).
    """
    ...
    def _call_gemini(self, prompt: str, is_json: bool = True) -> str:
        ...
        Lưu ý: Không dùng response_format của Langchain nếu proxy không support chuẩn, 
        chúng ta xử lý JSON qua parse thủ công.
        """
        try:
            # Thêm yêu cầu rõ ràng vào prompt nếu cần output dạng JSON
            if is_json and "JSON" not in prompt:
                prompt += "\n\nVUI LÒNG TRẢ VỀ KẾT QUẢ DƯỚI DẠNG ĐỊNH DẠNG JSON. KHÔNG THÊM BẤT KỲ VĂN BẢN NÀO BÊN NGOÀI JSON."
