...
try:
    ...
    def send_alert(msg, **kwargs):
        # Tránh lỗi in Unicode trên Windows Console không hỗ trợ utf-8 mặc định
        try:
            print(f"Alert: {msg}")
        except UnicodeEncodeError:
            print(f"Alert: {msg.encode('ascii', 'ignore').decode('ascii')}")
...
def backup_databases():
    ...
    try:
        ...
        # Cleanup: Giữ lại 5 file backup gần nhất (Tiết kiệm cho Boss)
        clean_old_backups(backup_dir, prefix="db_backup_*.zip", keep_last=5)
